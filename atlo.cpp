...
#endif
    cin>>n>>m; v.resize(n, vector<int>(m, 0)); w.resize(m);
    cerr<<" \t \t \t  "; fe(i, 0, n){if (i%10==0){cerr<<i/10<<" ";}else cerr<<"  ";}cerr<<endl;
    cerr<<" \t \t \t  "; fe(i, 0, n) cerr<<i%10<<" "; cerr<<endl;
    cerr<<" \t \t \t  "; fe(i, 0, n) cerr<<"--"; cerr<<endl;
    fe (i, 0, m) {
        cin>>x>>y; x--; y--;
        if (x>y) {z=x;x=y;y=z;}
        cerr<<">\t"<<x<<"\t"<<y<<"\t";
        cerr<<"] "; fe(i, 0, n)if (i==x || i==y){cerr<<"| ";}else if (x<i && i<y){cerr<<"--";}else cerr<<"  "; cerr<<endl;
        fe(j, 0, m)
            if (v[x][j] != v[y][j]) {
                if (w[j].first==x || w[j].second==y) continue;
                cerr<<"FAIL"<<endl;
                cout<<i+1<<endl;
                return 0;
            }
        for (int j=x+1; j%n!=y; j++) v[j%n][i]++;
        w[i]={x, y};
        cerr<<" \t \t \t] "; for(auto it:v) cerr<<accumulate(it.begin(), it.end(), 0)<<" "; cerr<<endl;
    }
    cerr<<"Thanks!"<<endl;
