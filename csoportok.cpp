// Feladat: Haladó / Sorozatok / Csoportok * 
// Lezsák Domonkos
    ...
    fv(i,v[0]) {
        fv(j,v[1]){
            fe(k,0,n) {
                ...
                if (v[0][i][k] && v[1][j][k]) {
                    fe(l,0,n) {
                        if (v[1][j][l]) if (!v[0][i][l]) goto nexti;
                    }
                }
                cerr<<"\033[32m"<<i<<"-"<<j<<"/"<<k+1<<" OK\033[0m"<<endl;
            }
            cerr<<"\033[33m"<<i<<"-"<<j<<" OK\033[0m"<<endl;
        }
        cerr<<"\033[34m"<<i<<" OK\033[0m"<<endl;
        cnt++;
nexti:
        cerr<<"\033[31mnexti\033[0m"<<endl;
        (void)0;
    }
    cout<<cnt<<endl;
