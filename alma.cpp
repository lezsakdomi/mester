...
int main(){
    ...
    for (auto& it : vv) {
        int buys = min(v[it.first], it.second);
        cerr<<" "<<it.first<<" "<<v[it.first]<<" "<<it.second<<" =>"<<buys<<endl;
        v[it.first] -= buys;
        it.second = buys;
    }

    {
        ...
        for (auto it : v) if (it.second) v_.push_back(it);
        cout<<v_.size()<<" "; for (auto it : v_) cout<<it.first<<" "<<it.second<<" "; cout<<endl;
    }
    {
        vector<pair<int, int>> v_;
        for (auto it : vv) if (it.second) v_.push_back(it);
        cout<<v_.size()<<" "; for (auto it : v_) cout<<it.first<<" "<<it.second<<" "; cout<<endl;
    }
