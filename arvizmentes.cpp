...
int main(){
    ...
    cerr<<"Thanks!"<<endl;
    if (!v.front()) vv.push_back({0,0});
    fe(i, 1, n){
        if (v[i]!=v[i-1]) {
            if (v[i]) vv.back().second=i;
            else vv.push_back({i, 0});
        }
    }
    if (!v.back()) vv.back().second=n;

    cout<<vv.size()<<endl;
    if (vv.size()) {
        long long int m=0;
        for (auto it : vv) cout<<it.first+1<<" "<<it.second<<" "; cout<<endl;
        for (auto it : vv) cout<<it.second-it.first<<" "; cout<<endl;
        for (auto it : vv) m+=it.second-it.first; cout<<(double)m/vv.size()<<endl;
    }
